...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Merged BED file from Step 1 "
             "(e.g. comvar_credible_footprint_overlap/GWFM_variants_in_K562.merged.hg38.bed)",
    )
    p.add_argument(
        "--allele-src", required=True,
        help="Directory of per-trait overlap CSVs or single CSV with SNP, A1, A2 columns "
             "(e.g. comvar_credible_footprint_overlap/K562.merged.hg38)",
    )
...
def main():
    ...
    print(f"Allele source: {args.allele_src}")
    print(f"Reference genome: {args.ref_genome}")
    print(f"JASPAR motifs: {args.jaspar_meme}")
    print(f"Sequence extension: {args.ext_bp} bp")
    print(f"FIMO threshold: {args.fimo_threshold}")
    print()

    # Load merged BED
    print("Loading merged BED ...")
    df = load_merged_bed(args.input_bed)
    print(f"  {len(df):,} variants")

    # Load allele info and annotate
    print("Loading allele info ...")
    ...
    n_missing = (df["A1"] == "").sum()
    if n_missing > 0:
        print(f"  WARNING: {n_missing:,} variants missing allele info, skipping")
        df = df[df["A1"] != ""].reset_index(drop=True)
    print(f"  {len(df):,} variants with allele info")
    print()

    # Load reference genome and motifs
    ...
    print(f"  Wrote {alt_fasta}")
    print()

    # FIMO motif scanning + TF change classification (corrected: best p-value per TF per variant)
    ...
    print()
    print(f"Results (best p-value per TF per variant): {len(result):,} variant-TF pairs")
